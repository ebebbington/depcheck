 ...
 */
function gatherImportsFromDepContent (lines: string[], fileImportsExtractedFrom: string): Imports {
  ...
  lines.forEach(line => {
    ...
    // Get imports
    const namedImportRegexOnNewLineMatch = line.match(namedImportRegexOnNewline)
    if (namedImportRegexOnNewLineMatch && namedImportRegexOnNewLineMatch.length) {
 ...
 */
async function iterateOverDirectoryAndCheckIfImportsAreUsed (dir: string, imports: Imports): Promise<Imports> {
  for await (const dirEntry of Deno.readDir(dir)) {
    if (dirEntry.name.indexOf(".ts") > 0 && dirEntry.name.indexOf("deps.ts") < 0) {
      const fileContent = decoder.decode(Deno.readFileSync(dir + "/" + dirEntry.name))
      ...
      await iterateOverDirectoryAndCheckIfImportsAreUsed(dir + "/" + dirEntry.name, imports)
    }
  }
  return imports
}
...
if ( (mainDepsContent.length === 1 && mainDepsContent[0] === "") !== true) { // not empty
  ...
  let mainImports: Imports = gatherImportsFromDepContent(mainDepsContent, "deps.ts")
...
const testDepsContent = decoder.decode(await Deno.readFile(`./${testDirName}/deps.ts`)).split("\n");
if ( (testDepsContent.length === 1 && testDepsContent[0] === "") !== true && testDirName) {
  // construct the imports
  let testImports: Imports = gatherImportsFromDepContent(testDepsContent, testDirName + "/deps.ts")
...
allImports = await iterateOverDirectoryAndCheckIfImportsAreUsed(".", allImports)

// check for any unused imports
allImports.forEach(imp => {
  if (imp.isUsed === false) {
    console.warn(colours.yellow(`Import "${imp.name}" is unused, originating from "${imp.file}"`))
  }
})
