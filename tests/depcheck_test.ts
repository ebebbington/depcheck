...
async function resetExampleProjectDir () {
  const p = Deno.run({
    cmd: ["cp", "-r", "tests/example_project_baseline", "tests/example_project"]
  })
  await p.status()
  p.close()
}
...
Rhum.testPlan("tests/depcheck_test.ts", () => {
  ...
  Rhum.testSuite("Running with no args", () => {
    Rhum.testCase("Warnings about unused dependencies are correct", async () => {
      const p = Deno.run({
  ...
  Rhum.testSuite("Running with --clean", () => {
    Rhum.testCase("Removes the unused dependencies", async () => {
      const p = Deno.run({
        cmd: ["deno", "run", "--allow-read='.'", "--allow-write='.'", "../../mod.ts", "--clean"],
        cwd: "./tests/example_project",
        stdout: "piped"
      })
      const status = await p.status()
  ...
  Rhum.testSuite("Running with --fmt", () => {
    Rhum.testCase("Formats code beforehand", async () => {
      const p = Deno.run({
        cmd: ["deno", "run", "--allow-read='.'", "--allow-run", "../../mod.ts", "--fmt"],
        cwd: "./tests/example_project",
        stdout: "piped"
      })
      const status = await p.status()
